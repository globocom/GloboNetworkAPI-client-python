...
class XMLErrorUtils(Exception):
    ...
    def __str__(self):
        msg = u'Erro ao criar ou ler o XML: Causa: %s, Mensagem: %s' % (self.cause, self.message)
...
def _add_text_node(value, node, doc):
    ...
    try:
        ...
        raise InvalidNodeTypeXMLError(t, u'Conteúdo de um Nó do XML com tipo de dado inválido: %s ' % value)
...
def _add_nodes_to_parent(map, parent, doc):
    ...
    for key, value in map.iteritems():
        try:
            ...
            raise InvalidNodeNameXMLError(i, u'Valor inválido para nome de uma TAG de XML: %s' % key)
...
def dumps(map, root_name, root_attributes=None):
    ...
    try:
        ...
        raise InvalidNodeNameXMLError(i, u'Valor inválido para nome de uma TAG de XML: %s' % root_name)
    finally:
        doc.unlink()
        ...
        for i in range(childs.length):
            ...
            if child.nodeType == Node.ELEMENT_NODE:
                if child.nodeName in childs_map:
                    ...
                    childs_map[child.nodeName] = _create_childs_map(child, force_list)
