...
class Usuario(GenericClient):
    def __init__(self, networkapi_url, user, password, user_ldap=None):
        ...
        """
        super(Usuario, self).__init__(networkapi_url, user, password, user_ldap)
    ...
    def get_by_id(self,id_user):
        ...
        if not is_valid_int_param(id_user):
            raise InvalidParameterError(u'The identifier of User is invalid or was not informed.')
    ...
    def change_password(self, id_user, user_current_password, password):
        ...
        if password is None or password == "":
            raise InvalidParameterError(u'A nova senha do usuário é inválida ou não foi informada')
        
        user_map = dict()
        user_map['user_id'] = id_user
        user_map['password'] = password
        
        code, xml = self.submit({'user':user_map}, 'POST', 'user-change-pass/')
