...
class Equipamento(GenericClient):
    def __init__(self, networkapi_url, user, password, user_ldap=None):
        ...
        """
        super(Equipamento, self).__init__(networkapi_url, user, password, user_ldap);
    ...
    def find_equips(self, name, iexact, environment, equip_type, group, ip, pagination):
        """
        ...
        key = "equipamento"
        return get_list_map(self.response(code, xml, [key, "ips", "grupos"]), key)
    ...
    def inserir(self, name, id_equipment_type, id_model, id_group):
        ...
        code, xml = self.submit({'equipamento':equip_map}, 'POST', 'equipamento/')
    ...
    def remover_ip(self, id_equipamento, id_ip):
        ...
        if not is_valid_int_param(id_ip):
            raise InvalidParameterError(u'O identificador do ip é inválido ou não foi informado.')
    ...
    def associate_ipv6(self, id_equip, id_ipv6):
        ...
        if not is_valid_int_param(id_ipv6):
            raise InvalidParameterError(u'The identifier of ip is invalid or was not informed.')            
    ...
    def remove_ipv6(self, id_equip, id_ipv6):
        ...
        url = 'ipv6/' + str(id_ipv6) + '/equipment/' + str(id_equip) + '/remove/'
    ...
    def listar_por_tipo_ambiente(self, id_tipo_equipamento, id_ambiente):
        ...
        if not is_valid_int_param(id_tipo_equipamento):
            raise InvalidParameterError(u'O identificador do tipo do equipamento é inválido ou não foi informado.')
        ...
        url = 'equipamento/tipoequipamento/' + str(id_tipo_equipamento) + '/ambiente/' + str(id_ambiente) + '/'
    ...
    def associar_grupo(self, id_equipamento, id_grupo_equipamento):
        ...
        code, xml = self.submit({'equipamento_grupo':equip_map}, 'POST', 'equipamentogrupo/')
    ...
    def remover_grupo(self, id_equipamento, id_grupo):
        ...
        if not is_valid_int_param(id_grupo):
            raise InvalidParameterError(u'O identificador do grupo é inválido ou não foi informado.')
        
        url = 'equipamentogrupo/equipamento/' + str(id_equipamento) + '/egrupo/' + str(id_grupo) + '/'
